...
支持的消息场景：
1. **预约确认提醒** - 当有人预约课程时，通知接收方确认
2. **预约成功通知** - 课程确认后，通知预约发起人
3. **课程取消通知** - 课程取消时通知另一方
4. **上课提醒（2小时前）** - 课程开始前2小时内提醒双方
5. **课程预约提醒（24小时内）** - 课程开始前不足24小时且大于2小时提醒双方
...
- **定时任务**：
  - 课时过期检查：每天凌晨2:00执行
  - 2小时上课提醒：每10分钟扫描一次（2小时内）
  - 24小时课程提醒：每小时扫描一次（2-24小时窗口，避免重复）
...
{
  time10: {
    value: timeSlot      // 上课时间段，格式：2025年12月22日 19:00~20:00
  },
  thing4: {
    value: addressName   // 上课地点，最大20字符
  }
...
**实现文件**:
- 服务类：`src/shared/services/bookingReminderService.js`
- 发送方法：`SubscribeMessageService.sendBookingReminderNotice()`

### 场景五：课程预约提醒（24小时内）

**模板ID**: `TEMPLATE_ID_HERE` （需配置）

**模板标题**: `课程预约提醒`

**触发时机**: 课程开始前"不足24小时且大于2小时"（待确认、已确认的课程均提醒）

**业务场景**:
- 学员课程提醒 → 通知学员
- 教练课程提醒 → 通知教练

**定时任务**: 每小时执行一次扫描（扫描2-24小时窗口内的课程，避免与2小时提醒重复）
...
```javascript
{
  ...
  thing7: {
    value: '不足24小时'    // 温馨提醒，固定内容
  }
}
```

**实现文件**:
- 服务类：`src/shared/services/bookingReminder24HService.js`
- 发送方法：`SubscribeMessageService.sendBookingReminder24HNotice()`
- 定时任务：`src/shared/app-common.js` 中的 `setupCronJobs()`
...
3. 字段配置：
   ...
   - `thing4`: 上课地点
   - `thing7`: 温馨提醒
4. 获取模板ID后，替换代码中的 `TEMPLATE_ID_HERE`
...
**触发时机**: 课程开始前 24 小时（待确认、已确认的课程均提醒）

**业务场景**:
- 学员课程提醒 → 通知学员
- 教练课程提醒 → 通知教练

**定时任务**: 每小时执行一次扫描（扫描23-25小时窗口内的课程）
