...
class CoachSettingController {
  ...
  static upsertSetting = asyncHandler(async (req, res) => {
    const coachId = req.user.id;
    const { completion_method } = req.body;

    if (!completion_method || !VALID_COMPLETION_METHODS.includes(completion_method)) {
    ...
    if (!created) {
      await setting.update({ completion_method });
    }

    logger.info('教练设置更新:', { coachId, completion_method, created });
